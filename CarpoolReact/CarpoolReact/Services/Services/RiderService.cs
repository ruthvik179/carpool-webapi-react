...
namespace CarpoolReact.Services
{
    public class RiderService : IRiderService
    {
        ...
        public RiderService(CarpoolContext carpoolDb, IConfiguration configuration)
        {
        ...
        public string RequestRide(ApplicationUser user, RideRquestModel model)
        {
            try
            {
                ...
                double price = Convert.ToDouble(Configuration["Charges:ChargePerKm"]);
                ...
                RideRequest rideRequest = new RideRequest(
                    ...
                    source.Id,
                    destination.Id,
                    Math.Round(model.Distance * price, 2, MidpointRounding.ToEven) 
                    );
        ...
        public List<MatchResponse> GetRequests(ApplicationUser user)
        {
            ...
            foreach (RideRequest request in requests)
            {
                ...
                MatchResponse match = new MatchResponse()
                {
                    ...
                    Status = Convert.ToString(request.RequestStatus)
                };
                matches.Add(match);
            }
            return matches;
        ...
        public string CancelBooking(ApplicationUser user, string bookingId)
        {
            try
            {
                ...
                Bill bill = carpoolDb.Bills.FirstOrDefault(c => c.Id.Equals(booking.BillId));
                double charge = Convert.ToDouble(Configuration["Charges:CancellationCharges"]);
        ...
        public string CancelRequest(ApplicationUser user, string requestId)
        {
            try
            {
                ...
                request.RequestStatus = RequestState.Cancelled;
